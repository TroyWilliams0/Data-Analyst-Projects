...
FROM movies3
GROUP BY runtime
ORDER BY `Total Popularity` DESC	-- There are 18,131 total popularity ratings
LIMIT 5;							-- Doing this gives us the top 5 runtimes with the most popularity ratings
...
LIMIT 5; 			-- Blue Beetle is the most popular with a rating of 2,994 and a runtime of 128
...
ORDER BY revenue DESC; 			-- Harry Potter and the Deathly Hallows: Part 2 made the most revenue in the Fantasy genre making $1,341,511,219
...
FROM movies3
GROUP BY `Month`
ORDER BY `Total Revenue` DESC;	-- December had the most revenue making $92,029,756,912
...
LIMIT 5;							-- 2017 had the most revenue made with $31,417,632,932  
...
-- 2018 accounts for 4.54% of all total revenue
...
FROM movies3
GROUP BY Season
ORDER BY `Total Revenue` DESC;	-- Based on revenue the movies released in the Summer are by far the money makers
...
ORDER BY revenue DESC
LIMIT 10;								-- Titanic is number 1 making $2,264,162,353 in 1997
...
ORDER BY revenue DESC					-- total top 5 revenue from 1960 and 2000 is $5,718,946,302
LIMIT 5;								-- Avatar is number 1 in revenue making $2,923,706,026 in 2009
...
ORDER BY revenue DESC				
LIMIT 5;								-- Avengers: Endgame is number 1 making $2,800,000,000 in 2019
